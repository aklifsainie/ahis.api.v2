...
**Status:** Partially implemented (2026-09-25). The endpoint and `Superadmin`-only policy are implemented and tested. The initial-role migration is intentionally not generated because discovery found that the runtime role store uses `IdentityRole`/`AspNetRoles`, while the existing explicit context mapping targets `IdentityRole<string>`/`IdentityRoles`; a corrective role-store blueprint is required before role data can be safely seeded.
...
This blueprint covers only the first P2 administrative Identity-control endpoint and the minimum authorization/initial-role seed required to secure it. It does not authorize the other administrative controls, role-management endpoints, user-role assignment, user provisioning, deployment, or applying the generated migration to a database.
...
Also run the planned authenticated integration/policy tests when available. Generate and inspect only the approved `SeedInitialIdentityRoles` migration after resolving the role-table discrepancy; do not run `dotnet ef database update`. Applying that migration remains a separate, named deployment authorization.

## Role provisioning and migration plan (Revision 2)

### Approved role policy direction

- Define exactly two initial human-user roles: `Superadmin` and `User`.
- Add a dedicated bearer-authenticated `IdentityAdminSecurityReadPolicy` that uses `RequireRole("Superadmin")`. `User` does not grant this endpoint permission.
- Access relies on the existing JWT issuance flow, which emits ASP.NET Core `ClaimTypes.Role` claims from `UserManager.GetRolesAsync`. A role assignment takes effect for a newly issued access token; no user is assigned `Superadmin` by this blueprint.
- The policy must not accept API keys or API-client permissions.

### Migration blueprint

**Context:** `IdentityContext` in `ahis.template.identity`.
...
**Proposed migration name:** `SeedInitialIdentityRoles`.

**Intended data change:** seed two deterministic `IdentityRole<string>` rows—`Superadmin` / `SUPERADMIN` and `User` / `USER`—with stable, documented non-secret IDs and concurrency stamps. The generated migration must insert only into the confirmed authoritative Identity role table and delete the same rows in `Down`. It must not assign either role to any user, add role claims, change role schema, alter user data, or modify unrelated migrations.

**Blocking prerequisite:** before generating the migration, inspect the current `IdentityContext` model/snapshot and generated migration output to resolve the discrepancy between the model's `IdentityRoles` mapping and the initial migration's extra `AspNetRoles` table plus `IdentityRoleClaims -> AspNetRoles` foreign key. If the generated migration reveals a table/relationship repair beyond two role inserts, stop; save a separate corrective migration blueprint and obtain approval. Do not hand-author an insert into either table to bypass this check.

**Exact generation command after approval and prerequisite resolution:**

```powershell
dotnet ef migrations add SeedInitialIdentityRoles --project .\ahis.template.identity\ahis.template.identity.csproj --startup-project .\ahis.template.api\ahis.template.api.csproj --context IdentityContext
```

Inspect every generated `Up`/`Down` operation and `IdentityContextModelSnapshot` change immediately after generation. Build the affected projects. Do not run `dotnet ef database update`; applying the migration is a separate, named deployment authorization.

### Additional files and tests

In addition to the files listed above:

- add a single role-name constants location owned by Identity or API authorization configuration, so seeding and the policy use the same exact `Superadmin`/`User` values;
- modify `IdentityContext` only if the authoritative mapping is confirmed and model seeding is the safe source of migration data;
- add migration/model verification appropriate to the resolved mapping, plus focused authorization tests showing a `Superadmin` token is admitted and a `User` token is denied.
