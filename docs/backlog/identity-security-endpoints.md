...
| Administrative Identity controls       | Partially implemented | Approved 2026-09-25 | 2026-09-25 | `GET /api/admin/identity/users/{userId}/security-state` requires `IdentityAdminSecurityReadPolicy` and the `Superadmin` role, returns a redacted security-state projection, and records a target-scoped view audit. Role data seeding is deferred: the runtime `IdentityRole` store uses `AspNetRoles`, while the explicit `IdentityRole<string>` mapping uses `IdentityRoles`; do not apply a role-seed migration until the role-store model is corrected and approved. |
