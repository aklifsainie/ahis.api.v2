...
Refresh tokens are stored in `IdentityContext` only as SHA-256 hashes. Each completed login has a random public-ID session; issued access tokens carry that opaque session ID, and rotations add a child token linked to the presented parent and require a conditional active-session update before storing a replacement token. `GET /api/account/sessions` returns only active sessions owned by the authenticated user and never returns refresh-token material; device, IP, and location data are not collected. Reuse of a revoked token invalidates active tokens and sessions for that user. `POST /api/account/sessions/revoke-all` requires an authenticated user and current step-up proof, revokes every session including the caller's, rotates the security version, and clears the refresh cookie after the Identity transaction commits. Logout revokes the current session and remains non-failing for missing, invalid, expired, or revoked supplied tokens. A daily hosted cleanup removes up to 500 leaf token rows retained for at least seven days after expiry, then up to 500 ended sessions retained for at least 30 days after expiry or revocation. The canonical refresh cookie is `Secure`, `HttpOnly`, `SameSite=Lax`, and scoped to `/api`; transition cleanup also covers legacy paths.
