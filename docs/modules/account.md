...
- `POST /api/admin/identity/users/{userId}/revoke-sessions` is a Superadmin-only administrative action protected by a separate bearer policy, the authenticated-security rate limit, and a current actor-bound five-minute proof. It validates that proof before target lookup, revokes existing targets regardless of active/deleted/locked status, rotates the target security version, and ends all target refresh sessions without notification. A successful self-revocation clears the actor's refresh cookie; audit delivery is best effort and excludes proof or token material.
- The administrative security-state projection derives its lockout flag from effective Identity restriction provenance. Administrative/Security Holds and `LegacyUnclassified` rows are not exposed as a new public field; hold placement, release, and classification remain unavailable as public routes.
